...
interface PermanentTooltipProps {
  text: string
...
export function PermanentTooltip({
  text,
  children,
  position = "top",
  className,
  ...
  return (
    <div className={cn("relative inline-block", className)}>
      ...
      <div className={getTooltipPosition()}>
        <div
          className={cn(
            "bg-gray-900 text-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-md sm:rounded-lg text-xs sm:text-sm font-medium shadow-lg border border-gray-700 whitespace-nowrap",
      ...
      {/* The element the tooltip points to */}
      {children}
    </div>
  )
}

// Chat-style permanent tooltip with WhatsApp icon
export function PermanentChatTooltip({
  text = "Chat with us!",
  children,
  position = "top",
  showIcon = true,
  ...
  return (
    <div className={cn("relative inline-block", className)}>
      {/* Permanent Chat Tooltip */}
      <div className={getTooltipPosition()}>
        <div
          className={cn(
            "bg-gray-900 text-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-md sm:rounded-lg text-xs sm:text-sm font-medium shadow-lg border border-gray-700 whitespace-nowrap flex items-center space-x-2",
            animate && "animate-fade-in-up"
          )}
        >
          {showIcon && (
...
// Floating chat widget with permanent tooltip (like the image you showed)
export function FloatingChatWidget({
  ...
  const getPositionClasses = () => {
    switch (position) {
      ...
      case "top-right":
        return "fixed top-4 right-4 md:top-8 md:right-8"
      case "top-left":
        return "fixed top-4 left-4 md:top-8 md:left-8"
      default:
        return "fixed bottom-4 right-4 md:bottom-8 md:right-8"
    }
  }
  ...
  return (
    <div className={cn(getPositionClasses(), "z-50", className)}>
      <PermanentChatTooltip text={text} position="top">
        <button
          onClick={onClick}
          className="bg-green-500 hover:bg-green-600 text-white rounded-full w-14 h-14 shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center btn-cursor-effect hover:scale-105"
        >
          <svg 
            xmlns="http://www.w3.org/2000/svg" 
            viewBox="0 0 448 512" 
